#!/bin/bash
#
# from https://github.com/LukeSmithxyz/voidrice/blob/master/.local/bin/mounter
...
# Check for phones.
phones="$(simple-mtpfs -l 2>/dev/null | sed "s/^/󰀲 /")"
...
normalparts="$(echo "$lsblkoutput"| grep -v crypto_LUKS | grep 'part\|rom\|crypt' | sed "s/^/󱊟 /" | filter )"
...
chosen="$(echo "$alldrives" | dmenu -l 10 -h 26 -p "Mount which drive?" -i -fn "Terminess Nerd Font")"

# Function for prompting user for a mountpoint.
getmount(){
	mp="$(find /mnt /media /mount /home -maxdepth 1 -type d 2>/dev/null | dmenu -i -p "Mount this drive where?" -fn "Terminess Nerd Font")"
	test -n "$mp"
	if [ ! -d "$mp" ]; then
		mkdiryn=$(printf "No\\nYes" | dmenu -i -p "$mp does not exist. Create it?" -fn "Terminess Nerd Font")
...
attemptmount(){
		...
		notify-send "󱊟 Drive Mounted." "$chosen mounted."
		exit
}
...
case "$chosen" in
	󱊟*)
		...
		notify-send "󱊟 Drive Mounted." "$chosen mounted to $mp."
		;;
	...
	*)
		...
		foot -W 80x1 -a "mount" -e sudo cryptsetup open "$chosen" "usb$num"
	...
	󰀲*)
		...
		notify-send "󰀲 Android Mounted." "Android device mounted to $mp."
		;;
esac
